...
class HlpFrontController {
	...
	*/
	...
	private function __construct($request = false)
	{
		...
		// очищаем от
		$request = preg_replace('@^/'.HLP_ADMIN_FOLDER.'@iu', '', $request);
		...
		for ($i=0; $i<$count; $i++) {
			$splits[$i] = HlpClear::text($splits[$i]);
		}
	...
	*/
	...
	{
		$controller =  'Hlp' . $this->_controller;
		$action 	= $this->_action;
		...
		if (class_exists($controller)) {
			...
			if ($class instanceof HlpIController) {
